       ...
       ans =  "Remainder Updated" + `\n` + "Object Id = " + doc._id + `\n Time : `+ updateData.message.date + " " + updateData.message.time + " " + updateData.message.isAM + `\n Message : ` + updateData.reminder;
        ...
        // let hanlde= keyWords[2]
            ...
            const left_date = left_arr.join("-");
                let response = await fetch(`http://localhost:3000/contest/?resource_id=${resource_id(keyWords[2])}&start__lte=${
                    left_date + "T00:00:00"
                  }&end__gte=${datetime}&order_by=start&duration__lt=999999`)
                let contest = await response.json()
                var ans = ""
                // console.log(data.Body)
                for( i = 0; i<contest.objects.length; i++) {
                   await WA.sendMessage(Json2Array(contest.objects[i]), data.From)
                }
