...
const syntaxContestDetails = ".cp contest <site-name> <past, present, future>"
const syntaxProfileDetails = ".cp profile <site-name> <handle>"
const syntaxContestReminder = ".cp rem <site-name> contest <contest_id>"
...
const runCode = `.run <language><space>\n<code>`;
...
const process = async (data) => {
  try {
    ...
    if (data.Body == ".help") {
      ...
      const url = `http://localhost:30000/youtube/?part=snippet&maxResults=${maxResults}&type=video&q=${search}`;
