  ...
  try {
    ...
    if (data.Body == ".help") {
      ...
      if(keyWords[2]=="contest"){
        ...
        let response = await fetch(`http://localhost:3000/contest?resource_id=${resource_id(keyWords[3])}&order_by=start&date=${date}`)
        ...
        for (i = 0; i < contest.objects.length; i++) {
            if(contest.objects[i].id==keyWords[4]){
                ...
                remDate=`${split[1]}/${splitDate[0]}/${split[0]} ${time}`
      ...
      const url = `http://localhost:3000/youtube/?part=snippet&maxResults=${maxResults}&key=${youtubeApi}&type=video&q=${search}`;
      ...
      if (keyWords[1] == "profile") {
        ...
        let response = await fetch(
          `http://localhost:3000/account/?handle__regex=${handle}&resource_id=${resource_id(
            keyWords[2]
          )}`
        );
        ...
        console.log(date);
        let response = await fetch(
          `http://localhost:3000/contest?resource_id=${resource_id(
            keyWords[2]
